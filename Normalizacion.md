...
COMIENZO 
ACEPTAR DATOS_VENTA
SI DATOS_VENTA.NRO_ASIENTO EXISTE ENTONCES 
    BUSCAR BOLETOS DONDE COD_VIAJE = DATOS_VENTA.COD_VIAJE Y NRO_ASIENTO = DATOS_VENTA.NRO_ASIENTO Y ESTADO = "VIGENTE"
    SI BOLETO EXISTE ENTONCES 
        EMITIR MENSAJE CON "EL ASIENTO NO SE ENCUENTRA DISPONIBLE"
    DE OTRO MODO 
    CREAR BOLETOS CON 
        COD_VIAJE = DATOS_VENTA.COD_VIAJE 
        DNI = "" ""
        APELLIDO = "" "" 
        NOMBRE = "" "" 
        ESTADO = "VIGENTE"
        NRO_ASIENTO = DATOS_VENTA.NRO_ASIENTO
    FIN CREAR
    EMITIR BOLETO CON 
        COD_VIAJE = DATOS_VENTA.COD_VIAJE
        APELLIDO = "" ""
        NOMBRE = "" "" 
        DNI = "" "" 
        BUSCAR VIAJE EN VIAJES DONDE COD_VIAJE = DATOS_VENTA.COD_VIAJE 
            FECHA_HORA_SALIDA = VIAJE.FH_SALIDA 
            NRO_PLATAFORMA = "" "" 
            PRECIO = "" "" 
        BUSCAR LOCALIDAD EN LOCALIDADES DONDE CODLOC = VIAJE.CODLOCORIGEN
            LOCAL_ORIGEN = LOCALIDAD.LOCALIDAD
        BUSCAR LOC EN LOCALIDADES DONDE CODLOC = VIAJE.CODLOCDESTINO
            LOCAL_DESTINO = LOCALIDAD.LOCALIDAD
        BUSCAR COLE EN COLECTIVOS DONDE N_COLECTIVO = VIAJE.N_COLECTIVO
        BUSCAR ASIENTO EN COLE DONDE NRO_ASIENTO = DATOS_VENTA.NRO_ASIENTO
            NRO_ASIENTO = ASIENTO.NRO_ASIENTO
            UBICACION = ASIENTO.UBICACION
    FIN EMITIR 
DE OTRO MODO 
    BUSCAR VIAJE EN VIAJES DONDE COD_VIAJE = DATOS_VENTA.COD_VIAJE
    BUSCAR COLE EN COLECTIVOS DONDE N_COLECTIVO = VIAJE.N_COLECTIVO
    CALCULAR BOLETO_GENERADO COMO 0 
    PARA CADA ASIENTO EN COLE HACER
        BUSCAR BOLETO EN BOLETOS DONDE COD_VIAJE = DATOS_VENTA.COD_VIAJE Y NRO_ASIENTO = ASIENTO.NRO_ASIENTO Y ESTADO = "VIGENTE"
        SI BOLETO NO EXISTE Y BOLETO_GENERADO = 0 ENTONCES 
            CREAR BOLETOS CON 
            .
            ...
            .
            FIN EMITIR 
            CALCULAR BOLETO_GENERADO = 0 ENTONCES 
                EMITIR MENSAJE CON "EL ASIENTO NO SE ENCUENTRA DISPONIBLE"
            FIN SI 
        FIN SI 
    FIN PARA CADA
...
COMIENZO 
ACEPTAR DATOS_DEVOLUCION 
    BUSCAR VIAJE EN VIAJES DONDE COD_VIAJE = DATOS_DEVOLUCION.COD_VIAJE 
    SI DIAS (VIAJE.FH_SALIDA - HOY()) > 15 ENTONCES 
        CALCULAR IMPORTE_R COMO VIAJE.PRECIO * 0.75
    DE OTRO MODO 
        SI DIAS (VIAJE.FH_SALIDA - HOY()) > 7 ENTONCES 
            CALCULAR IMPORTE_R COMO VIAJE.PRECIO * 0.5
        DE OTRO MODO 
            SI DIAS (VIAJE.FH_SALIDA - HOY()) > 3 ENTONCES 
                CALCULAR IMPORTE_R COMO VIAJE.PRECIO * 0.15
            DE OTRO MODO 
                CALCULAR IMPORTE_R COMO 0 
            FIN SI
    FIN SI 
    ACTUALIZAR BOLETOS CON IMPORTE_REINTEGRO = IMPORTE_R, ESTADO = "DISPONIBLE" DONDE COD_VIAJE = DATOS_DEVOLUCION.COD_VIAJE Y NRO_ASIENTO = DATOS_DEVOLUCION.NRO_ASIENTO Y ESTADO = "VIGENTE"

    EMITIR MSN REINTEGRO CON IMPORTE_R
    FIN EMITIR
FIN

EE5)
COMIENZO 
ACEPTAR SOL_INFORME_PASAJEROS 
EMITIR INFORME_PASAJEROS CON 
PARA CADA PASAJERO EN BOLETOS DONDE COD_VIAJE = SOL_INFORME_PASAJEROS.COD_VIAJE Y ESTADO = "VIGENTE" HACER 
    DNI = PASAJERO.DNI
    APELLIDO = "" ""
    NOMBRE = "" ""
FIN PARA CADA 
...
ET1) 
COMIENZO 
ACEPTAR MENSUALMENTE 
EMITIR INF_RECAUDACION CON 
    PARA CADA LOC EN LOCALIDADES HACER 
        COD_LOCALIDAD = LOC.COD_LOCALIDAD 
        LOCALIDAD = LOC.LOCALIDAD 
        CALCULAR TOTALFAC COMO 0
        CALCULAR TOTALREM COMO 0 
        PARA CADA VIAJE EN VIAJES DONDE MES(FH_SALIDA) = MES(HOY()) Y AÑO(FH_SALIDA) = AÑO(HOY()) Y CODLOCDESTINO = LOC.COD_LOCALIDAD HACER 
            PARA CADA BOL EN BOLETOS DONDE COD_VIAJE = VIAJE.COD_VIAJE HACER
                SI BOL.ESTADO = "VIGENTE" ENTONCES 
                    CALCULAR TOTALFAC COMO TOTALFAC + VIAJE.PRECIO 
                DE OTRO MODO 
                    CALCULAR TOTALREM COMO TOTALREM + (VIAJE.PRECIO - BOLETO.IMPORTE_REINTEGRO)
                FIN SI 
            FIN PARA CADA
        FIN PARA CADA
        RECAUDACION = TOTALFAC
        REMANENTE = TOTALREM 
    FIN PARA CADA
FIN EMITIR 
FIN 
